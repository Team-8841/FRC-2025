...
public class AutoIntakeSensorControl extends Command{

    private final Gripper m_gripper;
    private ElevatorSubsystem m_elevator;
    private boolean m_in, m_out;

    public AutoIntakeSensorControl(boolean in, boolean out, Gripper gripper, ElevatorSubsystem elevator){
    ...
    public boolean isFinished() {
        if(!m_elevator.getAlgaeState()){
            return m_gripper.isCoralDetected();
        } else if(m_elevator.getAlgaeState()) {
            return m_gripper.isAlgaeDetected();
        } else {
            return false;
        }
    }
